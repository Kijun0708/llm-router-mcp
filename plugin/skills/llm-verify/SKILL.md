...
consult_experts_parallel({
  experts: [
    ...
    { expert: "codereview", message: "다음을 분석해줘: [대상]" }
  ]
})
```
...
|----------|---------|---------|------|
| 아키텍처 결정 | `strategist` (GPT) | `metis` (GPT) | 서로 다른 전략적 관점 |
| 코드 품질 | `codereview` (Gemini) | `momus` (Gemini) | 이미 code-review 스킬에서 사용 |
| 보안 분석 | `security` (GPT) | `codereview` (Gemini) | 보안 전문 + 일반 리뷰 |
| DB 설계 | `data` (GPT) | `strategist` (GPT) | 실무 + 이론적 검증 |
